...
You can download the compiled binary:

- 123.0.6312.59 build
  - [win64](http://172.86.96.172/downloads/123.0.6312.59/hash-net-win-64-mini-installer-123.0.6312.59.exe)
  - [macos](http://172.86.96.172/downloads/123.0.6312.59/hash-net-macos-123.0.6312.59.dmg)

- 122.0.6248.1 build
  - [macos](http://172.86.96.172/downloads/hash-net.dmg)


(these are all http links, not https, so chrome can block downloading, just do right click on the link and select Save Link As) or tell chrome ``keep it`` if it asks.
...
After that, enter the key value into the settings (#Net tab, you'll see it) and save it.

Now you have to add a list of active #Net agents.
