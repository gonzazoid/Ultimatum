...
std::vector<uint8_t> GetHashOfMessage(std::string& message, const std::string& hash_func) {
  ...
  hash_checker->Update(message.data(), message.length());

  std::vector<uint8_t> hash = std::vector<uint8_t>(hash_length);
  hash_checker->Finish(hash.data(), hash_length);
  return hash;
}
...
bool SignMessage(const std::vector<uint8_t>& bytes, const std::string& private_key, std::string& output) {
  ...
  auto root = base::JSONReader::ReadAndReturnValueWithError(base::StringPiece(reinterpret_cast<const char *>(bytes.data()), bytes.size()));
...
base::Value::List VerifySignedResponses(const std::string& msg, const std::string& private_key, const GURL& url) {
  /* validation */
  auto root = base::JSONReader::ReadAndReturnValueWithError(base::StringPiece(msg.data(), msg.length()));
...
base::Value::List VerifyRelatedResponses(std::string msg, const std::string& private_key, const GURL& url) {
  /* validation */
  auto root = base::JSONReader::ReadAndReturnValueWithError(base::StringPiece(msg.data(), msg.length()));
