...
bool SignMessage(const std::vector<uint8_t>& bytes, const std::string& private_key, std::string& output) {
  ...
  auto root = base::JSONReader::ReadAndReturnValueWithError(std::string_view(reinterpret_cast<const char *>(bytes.data()), bytes.size()));
...
base::Value::List VerifySignedResponses(const std::string& msg, const std::string& private_key, const GURL& url) {
  /* validation */
  auto root = base::JSONReader::ReadAndReturnValueWithError(std::string_view(msg.data(), msg.length()));
