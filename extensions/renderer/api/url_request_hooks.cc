...
UrlRequestHooks::UrlRequestHooks() = default;
UrlRequestHooks::~UrlRequestHooks() = default;

bool UrlRequestHooks::CreateCustomEvent(v8::Local<v8::Context> context,
                                        const std::string& event_name,
                                        v8::Local<v8::Value>* event_out) {
  // Don't create a custom event for the "onActionIgnored" event.
  // if (event_name == api::url_request::OnActionIgnored::kEventName)
  //   return false;
  ...
  v8::Local<v8::Value> get_event_value;
  {
    ...
    if (!internal_bindings
             ->Get(context,
                   gin::StringToSymbol(isolate, "createUrlRequestEvent"))
      ...
      // gracefully.
      LOG(ERROR) << "Unexpected error when creating custom urlRequest event: "
                 << "`createUrlRequestEvent` not found on internal bindings.";
      // Even though this failed, we return true because a custom event was
      // supposed to be created.
      return true;
    }
  ...
  if (!JSRunner::Get(context)
    ...
    LOG(ERROR) << "Unexpected error when creating custom urlRequest event: "
               << "`createUrlRequestEvent` function did not successfully run.";
