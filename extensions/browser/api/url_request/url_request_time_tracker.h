...
#ifndef EXTENSIONS_BROWSER_API_URL_REQUEST_URL_REQUEST_TIME_TRACKER_H_
#define EXTENSIONS_BROWSER_API_URL_REQUEST_URL_REQUEST_TIME_TRACKER_H_
...
// by using the urlRequest API.
class ExtensionUrlRequestTimeTracker {
 public:
  ExtensionUrlRequestTimeTracker();

  ExtensionUrlRequestTimeTracker(const ExtensionUrlRequestTimeTracker&) =
      delete;
  ExtensionUrlRequestTimeTracker& operator=(
      const ExtensionUrlRequestTimeTracker&) = delete;

  ~ExtensionUrlRequestTimeTracker();

  // Records the time that a request was created.  |has_listener| will be true
  // if there is at least one urlRequest listener registered.
 ...
 private:
  // FRIEND_TEST_ALL_PREFIXES(ExtensionUrlRequestTimeTrackerTest, Histograms);
...
#endif  // EXTENSIONS_BROWSER_API_URL_REQUEST_URL_REQUEST_TIME_TRACKER_H_
