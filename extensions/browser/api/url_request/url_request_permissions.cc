...
// ExtensionUrlRequestEventRouter::RequestFiler, which specifies the schemes
// allowed by web request event listeners. Consolidate the two.
bool HasUrlRequestScheme(const GURL& url) {
...
bool IsUrlRequestResourceTypeFrame(
    extensions::UrlRequestResourceType url_request_type) {
  return url_request_type == extensions::UrlRequestResourceType::MAIN_FRAME ||
         url_request_type == extensions::UrlRequestResourceType::SUB_FRAME;
}
...
PermissionsData::PageAccess CanExtensionAccessURLInternal(
    ...
    UrlRequestPermissions::HostPermissionsCheck host_permissions_check,
    const std::optional<url::Origin>& initiator,
    const std::optional<extensions::UrlRequestResourceType>& url_request_type) {
  const extensions::Extension* extension =
  ...
  switch (host_permissions_check) {
    case UrlRequestPermissions::DO_NOT_CHECK_HOST:
      return PermissionsData::PageAccess::kAllowed;
    case UrlRequestPermissions::REQUIRE_HOST_PERMISSION_FOR_URL: {
      PermissionsData::PageAccess access =
          GetHostAccessForURL(*extension, url, tab_id);

      bool is_navigation_request =
          url_request_type && IsUrlRequestResourceTypeFrame(*url_request_type);
      ...
      // extensions with urlRequest to work well with runtime host permissions.
    ...
    case UrlRequestPermissions::REQUIRE_HOST_PERMISSION_FOR_URL_AND_INITIATOR: {
      ...
      // with urlRequest to work well with runtime host permissions. See
      // crbug.com/851722.

      return GetHostAccessForURL(*extension, initiator->GetURL(), tab_id);
    }
    case UrlRequestPermissions::REQUIRE_ALL_URLS:
...
bool IsSensitiveGoogleClientUrl(const extensions::UrlRequestInfo& request) {
  return false;
...
bool IsMainFrameNavigationRequest(const extensions::UrlRequestInfo& request) {
  return request.is_navigation_request &&
         request.url_request_type ==
             extensions::UrlRequestResourceType::MAIN_FRAME;
}

}  // namespace

// static
bool UrlRequestPermissions::HideRequest(
    extensions::UrlRequestPermissionHelper* permission_helper,
    const extensions::UrlRequestInfo& request) {
  if (!HasUrlRequestScheme(request.url))
    return true;
  ...
  if (is_request_from_browser) {
    ...
    if (request.is_service_worker_script) {
      DCHECK(request.url_request_type ==
             extensions::UrlRequestResourceType::SCRIPT);
      return false;
    }
    ...
    DCHECK(request.url_request_type ==
               extensions::UrlRequestResourceType::MAIN_FRAME ||
           request.url_request_type ==
               extensions::UrlRequestResourceType::SUB_FRAME ||
           request.url_request_type ==
               extensions::UrlRequestResourceType::OBJECT);
  ...
  if (request.initiator.has_value() &&
    ...
    // The call to `HasUrlRequestScheme()` with an early exit at the top already
...
// static
PermissionsData::PageAccess UrlRequestPermissions::CanExtensionAccessURL(
    extensions::UrlRequestPermissionHelper* permission_helper,
    const extensions::ExtensionId& extension_id,
    const GURL& url,
    int tab_id,
  ...
  return CanExtensionAccessURLInternal(
      ...
      host_permissions_check, initiator, url_request_type);
}

// static
bool UrlRequestPermissions::CanExtensionAccessInitiator(
    extensions::UrlRequestPermissionHelper* permission_helper,
    const extensions::ExtensionId extension_id,
    const std::optional<url::Origin>& initiator,
    int tab_id,
  ...
  return CanExtensionAccessURLInternal(
             ...
             crosses_incognito,
             UrlRequestPermissions::REQUIRE_HOST_PERMISSION_FOR_URL,
