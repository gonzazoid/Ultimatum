...
#ifndef EXTENSIONS_BROWSER_API_URL_REQUEST_URL_REQUEST_PROXYING_URL_LOADER_FACTORY_H_
#define EXTENSIONS_BROWSER_API_URL_REQUEST_URL_REQUEST_PROXYING_URL_LOADER_FACTORY_H_
...
// Owns URLLoaderFactory bindings for UrlRequest proxies with the Network
// Service enabled.
class UrlRequestProxyingURLLoaderFactory
    : public UrlRequestAPI::Proxy,
 ...
 public:
  class InProgressRequest : public network::mojom::URLLoader,
   ...
   public:
    ...
    // For CORS preflights
    InProgressRequest(UrlRequestProxyingURLLoaderFactory* factory,
                      uint64_t request_id,
                      int32_t frame_routing_id,
                      const network::ResourceRequest& request);
   ...
   private:
    ...
    const raw_ptr<UrlRequestProxyingURLLoaderFactory> factory_;
    ...
    std::optional<UrlRequestInfo> info_;

    mojo::Receiver<network::mojom::URLLoaderClient> proxied_client_receiver_{
        this};
    mojo::Remote<network::mojom::URLLoader> target_loader_;

    // NOTE: This is state which ExtensionUrlRequestEventRouter needs to have
    ...
    // ExtensionUrlRequestEventRouter) through much of the request's lifetime.
    network::mojom::URLResponseHeadPtr current_response_;
    mojo::ScopedDataPipeConsumerHandle current_body_;
    std::optional<mojo_base::BigBuffer> current_cached_metadata_;
    GURL redirect_url_;
    extension_url_request_api_helpers::BlockingResponse response_;
  ...
  UrlRequestProxyingURLLoaderFactory(
      const UrlRequestProxyingURLLoaderFactory&) = delete;
  UrlRequestProxyingURLLoaderFactory& operator=(
      const UrlRequestProxyingURLLoaderFactory&) = delete;

  ~UrlRequestProxyingURLLoaderFactory() override;
 ...
 private:
  ...
  const raw_ptr<UrlRequestAPI::RequestIDGenerator> request_id_generator_;
  ...
  // Owns |this|.
  const raw_ptr<UrlRequestAPI::ProxySet> proxies_;
  ...
  std::map<int32_t, uint64_t> network_request_id_to_url_request_id_;
  ...
  base::WeakPtrFactory<UrlRequestProxyingURLLoaderFactory> weak_factory_{this};
};

}  // namespace extensions

#endif  // EXTENSIONS_BROWSER_API_URL_REQUEST_URL_REQUEST_PROXYING_URL_LOADER_FACTORY_H_
