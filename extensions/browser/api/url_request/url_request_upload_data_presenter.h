...
#ifndef EXTENSIONS_BROWSER_API_URL_REQUEST_UPLOAD_DATA_PRESENTER_H_
#define EXTENSIONS_BROWSER_API_URL_REQUEST_UPLOAD_DATA_PRESENTER_H_
...
class UrlRequestFormDataParser;
}
...
class UrlRequestUploadDataPresenter {
 public:
  UrlRequestUploadDataPresenter(const UrlRequestUploadDataPresenter&) = delete;
  UrlRequestUploadDataPresenter& operator=(const UrlRequestUploadDataPresenter&) = delete;

  virtual ~UrlRequestUploadDataPresenter();
...
// for that file.
class UrlRequestRawDataPresenter : public UrlRequestUploadDataPresenter {
 public:
  UrlRequestRawDataPresenter();

  UrlRequestRawDataPresenter(const UrlRequestRawDataPresenter&) = delete;
  UrlRequestRawDataPresenter& operator=(const UrlRequestRawDataPresenter&) = delete;

  ~UrlRequestRawDataPresenter() override;

  // Implementation of UrlRequestUploadDataPresenter.
...
// parser classes inheriting from UrlRequestFormDataParser to parse the concatenated
...
class UrlRequestParsedDataPresenter : public UrlRequestUploadDataPresenter {
 public:
  explicit UrlRequestParsedDataPresenter(const net::HttpRequestHeaders& request_headers);

  UrlRequestParsedDataPresenter(const UrlRequestParsedDataPresenter&) = delete;
  UrlRequestParsedDataPresenter& operator=(const UrlRequestParsedDataPresenter&) = delete;

  ~UrlRequestParsedDataPresenter() override;
  ...
  // Allows to create UrlRequestParsedDataPresenter without request headers. Uses the
  // parser for "application/x-www-form-urlencoded" form encoding. Only use this
  // in tests.
  static std::unique_ptr<UrlRequestParsedDataPresenter> CreateForTests();

 private:
  // This constructor is used in CreateForTests.
  explicit UrlRequestParsedDataPresenter(const std::string& form_type);

  // Clears resources and the success flag.
  void Abort();

  std::unique_ptr<UrlRequestFormDataParser> parser_;
  bool success_;
...
#endif  // EXTENSIONS_BROWSER_API_URL_REQUEST_UPLOAD_DATA_PRESENTER_H_
